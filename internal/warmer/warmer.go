...
func (w *Warmer) prefetchSegment(ctx context.Context, seg index.Segment) {
	...
	if seg.IVFKeys != nil {
		...
		if seg.IVFKeys.CentroidsKey != "" {
			w.prefetchToCache(ctx, seg.IVFKeys.CentroidsKey, true)
		}
		// Cluster offsets - small file, cacheable
		if seg.IVFKeys.ClusterOffsetsKey != "" {
			w.prefetchToCache(ctx, seg.IVFKeys.ClusterOffsetsKey, true)
		}
		// Note: We don't prefetch ClusterDataKey as it's large
	}

	// Prefetch filter bitmap keys
	for _, filterKey := range seg.FilterKeys {
		w.prefetchToCache(ctx, filterKey, false)
	}

	// Prefetch docs key (column headers)
	if seg.DocsKey != "" {
		w.prefetchToCache(ctx, seg.DocsKey, false)
	}
}

// prefetchToCache fetches an object and caches it.
func (w *Warmer) prefetchToCache(ctx context.Context, key string, toRAM bool) {
	if w.store == nil {
		return
	}
	...
	if toRAM && w.ramCache != nil {
		...
		ramCacheKey := cache.MemoryCacheKey{
			...
			ShardID:   "warm",
			ItemID:    key,
			ItemType:  cache.TypeCentroid, // Use centroid type for IVF data
		}
